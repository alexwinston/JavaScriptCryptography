class Eyes {
    static base(s, n, p = 3) {
        return s.map(v => (v.charCodeAt(0) - 32).toString(n).padStart(p,'0'))
    }
}
...
function decode(pattern = 10, positions = [0,1,2,3,4], alphabet = abc83, ct = m, debug = false) {
    ...
    for (var i = 0; i < ct.length; i++) {
        const a = ascii(rotate(encode(parse(ct[i][0], ct[i][1], positions))))
        // console.log(a)
        t.push(a)
    }
...
// https://discord.com/channels/453998283174576133/817530812454010910/1158836082230706246
// const pt = [
...
// const pt = [
//     "1THIS_REACTION_IS_BANNED_COPPER+[FROZEN_ALUMINIUM]_OXIDE+OIL>SUNNY_ALUMINIUM_OXIDE_(INACTIVE)+NOEYES",
//     "2THIS_REACTION_IS_BANNED_SILVER+[MOLTEN_ALUMINIUM]_(INACTIVE)>[SUNNY_ALUMINIUM]_(INACTIVE)+VOID_NOITAEYES",
//     "3THIS_REACTION_IS_BANNED_ELECTRICITY+[MOLTEN_ALUMINIUM]_(ACTIVE)+PLAYER>[SUNNY_ALUMINIUM]_(ACTIVE)+INTERMEDIATE+THENNOITA",
//     "4THIS_NONALCHEMY_IS_ALLOWED_NAVY+[FROZEN_ALUMINIUM]_OXIDE+OIL>FROZEN_ALUMINIUM_OXIDE_(INACTIVE)+SALTTRUSTY",
//     "5THIS_NONUNKNOWN_IS_BANNED_SILVER+[MOLTEN_ALUMINIUM]_(INACTIVE)>[MOLTEN_ALUMINIUM]_(INACTIVE)+VOID_LIQUIDTHISISATESTOFTHEMERGENCYBROADCAST",
//     "6THIS_NONCHEMISTRY_IS_BANNED_ELECTRICITY+[MOLTEN_ALUMINIUM]_(ACTIVE)+PLAYER>[MOLTEN_ALUMINIUM]_(ACTIVE)+INTERMEDIATE+SMOKEEYES",
//     "7THIS_REACTION_IS_NOTBANNED_COPPER+[FROZEN_NEON]_OXIDE+OIL>HELLO_NEON_OXIDE_(INACTIVE)+SALT[HELLO_ALUMINIUM]_(ACTIVE)+NOW",
//     "8THIS_REACTION_IS_NOTBANNED_SILVER+[MOLTEN_NEON]_(INACTIVE)>[HELLO_NEON]_(INACTIVE)+VOID_LIQUID[HELLO_ALUMINIUM]_(NOW)EYES",
//     "9THIS_REACTION_IS_NOTBANNED_ELECTRICITY+[MOLTEN_NEON]_(ACTIVE)+PLAYER>[HELLO_NEON]_(ACTIVE)+INTERMEDIATE+KNOWLEDGE",
// ]
...
const pt = [
    "OTAOSEVIGSNTTSIEZDEXAIEOAYZMSZFEAMOTHEFTONONEOEOEOTIAGANRTJOMSFHIAHETEURAUETAOMNYTPSYZEENIIEJRIETNO",
    "YTAOSEVIGSNTTSIEZDEXAIEOAIEIOZFEAHHPEEFTONONEOEOEONHRWTNYITTAIUGTBJHTZTNELEEFTEDNTRKSZEEUSAETTLNENTSDKE",
    "LTAOSEVIGSNTTSIEZDEXAIEOATWODOTYFTRMOXAEREOMYBETIOTSBOIEEENGHWESZSKPTFEATWNLIFAEMTREGRTMGETNAAHXDAPNWHTSENPOESHEHNOSLK",
    "WTAOVPTNIANSHOOYIGQSOEFEOPKHLERQEDDSEZDNISOZNYANTAYWOXKHSEEOSTOZETSEOEOTHGIITGKIOBAHDLONHWTTYECNSEOSSE",
    "STAOVPASIMXOSVTTSATCOENURMYESEZFKOMLAROIWAYVARTEFDMTNVQLSNEAFRABRDTARRSTHYCEETATUITEEITLEEFSENTRMZZFXLOEABEALYSATTENARRIETRENXTAANETUHKDG",
    "KTAOVPGUEEIHEOTRLSKIUEEIEEENEZNASDNOIOWRANOETYFINEILHIMTRKOETTDBNNELONEEJIKSLALALQLEMHDFETAYNTETEOATSMFEKOSLEATOOOEULVUPGIPT",
    "HTAOVPASIMAQDERETALAOWNLAEIWAKSSRYEPYVOPQESTEOOTVAEONRTTYHLNUOTOKNXHYRARFSEVNSRTSNOJMTYWCAHSIXHRTMAGHETNLTELTSHBYTTWRXE",
    "WTAOVPASIMAQDERETALAOSFYATQEZKREATEHLEUZLEXAHTNTTLZGGORITVHTPRROTUEFDIHSALENTOOKTGUIPLWTESYBFFOTUFAETLIFSEZNIAENKAIPNEMI",
    "JTAOVPASIMAQDERETALAOJFRNEIWRKSSYHEPAOOPQOSTTERFROTOCKIIIEHDKNCITQKVPDBSAGHNNYHEFWEOSFMDMENUDJTUBIFTAMSONTTUSHSNGM"
]